...
async function handleMessage(e) {
    ...
    try {
        if (!file) {
            let { data } = await axios.post('http://localhost:3000/message/', { msg, groupId }, { headers: { "Authorization": token } })
            ...
            let { data } = await axios.post(`http://localhost:3000/file/${groupId}`, formData, { headers: { "Authorization": token, 'Content-Type': 'multipart/form-data' } })
...
document.addEventListener('DOMContentLoaded', async () => {
    let groups;
    try {
        let { data } = await axios.get('http://localhost:3000/group/all', { headers: { "Authorization": token } })
        groups = data.groups
    } catch (err) {
        console.log(err)
    }
    ...
    lists.forEach((list, ind1) => {
        list.addEventListener('click', async () => {
            ...
            let messages;
            try {
                let { data } = await axios.get(`http://localhost:3000/message/${groupId}`, { headers: { "Authorization": token } })
