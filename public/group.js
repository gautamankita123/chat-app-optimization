...
document.addEventListener('DOMContentLoaded', async (e) => {
    ...
    let users;
    try {
        let { data } = await axios.get('http://localhost:3000/user/all', { headers: { "Authorization": token } })
...
async function postGroupDetails(groupDetails) {
    ...
    try {
        const response = await axios.post('http://localhost:3000/group/create', groupDetails, { headers: { "Authorization": token } })
