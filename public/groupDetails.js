...
async function handleDomLoaded(e) {
    ...
    let users;
    try {
        let { data } = await axios.get(`http://localhost:3000/group/members/${groupId}`, { headers: { "Authorization": token } })
    ...
    try {
        if (isRootUserAdmin) {
            if (classVal == 'make btn btn-secondary float-right') {
                let { data } = await axios.post(`http://localhost:3000/group/makeadmin`, details, { headers: { "Authorization": token } })
                handleDomLoaded(e)
            }
            else if (classVal == 'dismiss btn btn-secondary float-right') {
                let { data } = await axios.post(`http://localhost:3000/group/dismissadmin`, details, { headers: { "Authorization": token } })
                handleDomLoaded(e)
            }
            else if (classVal == 'remove btn btn-secondary mr-2 float-right') {
                let { data } = await axios.post(`http://localhost:3000/group/removemember`, details, { headers: { "Authorization": token } })
                handleDomLoaded(e)
            }
        }
    } catch (err) {
